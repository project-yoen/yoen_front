...
class _TravelPrepaymentCreateScreenState
  ...
  List<SettlementParticipant> _settlementParticipants = [];
  List<XFile> _images = [];
  ...
  Future<void> _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (!_isPreSettled && _settlementParticipants.isEmpty) {
        ScaffoldMessenger.of(
          context,
      ...
      ref
          ...
          .then((_) {
            if (ref.read(paymentNotifierProvider).createStatus ==
                Status.success) {
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: '결제이름'),
                  validator: (value) => value!.isEmpty ? '결제이름을 입력하세요.' : null,
                ),
                const SizedBox(height: 16.0),
                TextFormField(
                  ...
                  validator: (value) => value!.isEmpty ? '결제금액을 입력하세요.' : null,
                ),
                const SizedBox(height: 16.0),
                _buildCurrencySelector(),
                const SizedBox(height: 16.0),
                _buildPreSettledCheckbox(),
                if (!_isPreSettled) ...[
                  const SizedBox(height: 16.0),
  ...
  Widget _buildPreSettledCheckbox() {
  ...
  Widget _buildSettlementUserSelector() {
    return TextFormField(
      ...
      validator: (value) {
        if (!_isPreSettled && (value == null || value.isEmpty)) {
  ...
  void _showSettlementUserDialog() async {
    final result = await showDialog<List<SettlementParticipant>>(
      context: context,
    ...
    if (result != null) {
      setState(() {
        ...
        _settlementUsersController.text = result
